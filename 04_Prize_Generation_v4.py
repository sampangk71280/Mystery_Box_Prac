...
for item in range(0, NUM_TRIALS):
    ...
    for thing in range(0,3):
        # randit finds numbers between given endpoints, including both endpoints
        prize_num = random.randint(1, 10)
        # prize += " "
        if prize_num  == 1:
            # one in ten chances of getting gold
            prize += "gold"
            round_winnings += 5
        elif 1 < prize_num <= 3:
            ...
            # copper if its 4, 5, 6 or 7, <40% chance getting copper>
            #prize += "copper"
            round_winnings += 1
        """else:
            prize += "lead"""
